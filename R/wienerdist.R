...
{
  check_wiener_pars(alpha,tau,beta,delta) 
  stopifnot(is.numeric(q) && (is.character(resp) || is.factor(resp)))
...
{
  check_wiener_pars(alpha,tau,beta,delta) 
  stopifnot(is.numeric(p) , (is.character(resp) || is.factor(resp))) 
...
wiener_likelihood <- function(x, dat) {
 check_wiener_pars(x[1],x[2],x[3],x[4])
